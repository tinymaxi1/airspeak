/**
 * Admin manuel aylık lig rotation tetikleyici.
 *
 * POST /api/admin/leagues/rotate-monthly
 * Yetki: super_admin
 */
...
export async function POST(): Promise<NextResponse> {
  ...
  const url = `${supabaseUrl}/functions/v1/league-monthly-rotation`;
  ...
  try {
    ...
    // Fallback: SQL function direkt
    const supabase = createServiceClient();
    const { data, error } = await (supabase as any).rpc('rotate_monthly_championships');
    if (error) {
      return NextResponse.json(
        {
          ok: false,
