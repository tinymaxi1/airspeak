/**
 * Generic CRUD form'ları için tablo şemaları.
 * Her tablo için fields + slug stratejisi tanımlı.
 */
import type { TableSchema } from '@/components/forms/GenericTableForm';

export const ICAO4_SCHEMA: TableSchema = {
  table: 'icao4_questions',
  title: 'ICAO 4 Sorusu',
  description: '3 set × 4 bölüm × ~50 soru (vocab/phraseology/listening/reading)',
  slugFromKey: 'question',
  slugPrefix: 'icao4_set{set_no}_{section}',
  revalidate: '/icao4',
  fields: [
    {
      key: 'set_no',
      label: 'Set',
      type: 'select',
      required: true,
      defaultValue: '1',
      options: [
        { id: '1', label: 'Set 1' },
        { id: '2', label: 'Set 2' },
        { id: '3', label: 'Set 3' },
      ],
    },
    {
      key: 'section',
      label: 'Bölüm',
      type: 'select',
      required: true,
      defaultValue: 'vocabulary',
      options: [
        { id: 'vocabulary', label: 'Vocabulary' },
        { id: 'phraseology', label: 'Phraseology' },
        { id: 'listening', label: 'Listening' },
        { id: 'reading', label: 'Reading' },
        { id: 'grammar', label: 'Grammar' },
        { id: 'critical', label: 'Critical' },
      ],
    },
    {
      key: 'level',
      label: 'Seviye',
      type: 'select',
      required: true,
      defaultValue: 'B1',
      options: [
        { id: 'B1', label: 'B1' },
        { id: 'B2', label: 'B2' },
        { id: 'B2+', label: 'B2+' },
        { id: 'C1', label: 'C1' },
      ],
    },
    {
      key: 'question',
      label: 'Soru (EN)',
      type: 'textarea',
      required: true,
      placeholder: 'What does ATC mean?',
      rows: 2,
    },
    {
      key: 'question_tr',
      label: 'Soru (TR)',
      type: 'textarea',
      hint: 'Türkçe — opsiyonel',
      rows: 2,
    },
    {
      key: 'context',
      label: 'Bağlam (listening transcript)',
      type: 'textarea',
      rows: 3,
      hint: 'Sadece listening için',
    },
    {
      key: 'audio_url',
      label: 'Ses URL',
      type: 'text',
      hint: 'Faz 5 audio pipeline ile doldurulur',
    },
    {
      key: 'options',
      label: 'Şıklar (JSON)',
      type: 'json',
      required: true,
      rows: 6,
      placeholder: '[{"id":"a","text":"…"},{"id":"b","text":"…"},{"id":"c","text":"…"},{"id":"d","text":"…"}]',
    },
    {
      key: 'correct_id',
      label: 'Doğru cevap (id)',
      type: 'text',
      required: true,
      placeholder: 'a',
    },
    {
      key: 'explanation_tr',
      label: 'Açıklama (TR)',
      type: 'textarea',
      rows: 3,
    },
  ],
};

export const ORAL_SCHEMA: TableSchema = {
  table: 'oral_prompts',
  title: 'Sözlü Sınav Prompt',
  description: 'ICAO sözlü sınav için 4 görev tipi prompt',
  slugFromKey: 'prompt',
  slugPrefix: 'oral_{task_type}',
  revalidate: '/oral',
  fields: [
    {
      key: 'task_type',
      label: 'Görev tipi',
      type: 'select',
      required: true,
      defaultValue: 'picture_description',
      options: [
        { id: 'picture_description', label: 'Resim tasviri' },
        { id: 'story_telling', label: 'Hikâye anlatma' },
        { id: 'problem_solving', label: 'Problem çözme' },
        { id: 'common_topics', label: 'Genel konular' },
      ],
    },
    {
      key: 'level',
      label: 'Seviye',
      type: 'select',
      required: true,
      defaultValue: 'B2',
      options: [
        { id: 'B1', label: 'B1' },
        { id: 'B2', label: 'B2' },
        { id: 'B2+', label: 'B2+' },
        { id: 'C1', label: 'C1' },
      ],
    },
    {
      key: 'prompt',
      label: 'Prompt (EN)',
      type: 'textarea',
      required: true,
      rows: 3,
    },
    {
      key: 'prompt_tr',
      label: 'Prompt (TR)',
      type: 'textarea',
      rows: 3,
    },
    {
      key: 'cues_tr',
      label: 'İpuçları (TR)',
      type: 'list',
      hint: 'Her satıra bir bullet ipucu',
      rows: 4,
    },
    {
      key: 'vocabulary_tr',
      label: 'Önerilen kelimeler',
      type: 'list',
      rows: 3,
    },
    {
      key: 'image_url',
      label: 'Görsel URL',
      type: 'text',
    },
    {
      key: 'preparation_seconds',
      label: 'Hazırlık (sn)',
      type: 'number',
      defaultValue: 30,
    },
    {
      key: 'speaking_seconds',
      label: 'Konuşma (sn)',
      type: 'number',
      defaultValue: 90,
    },
  ],
};

export const PLACEMENT_SCHEMA: TableSchema = {
  table: 'placement_questions',
  title: 'Placement Test Sorusu',
  description: '4 boyutta seviye ölçümü (general_english, aviation_english, knowledge, communication)',
  slugFromKey: 'question',
  slugPrefix: 'placement_{dimension}_{level}',
  revalidate: '/placement',
  fields: [
    {
      key: 'dimension',
      label: 'Boyut',
      type: 'select',
      required: true,
      defaultValue: 'general_english',
      options: [
        { id: 'general_english', label: 'Genel İngilizce' },
        { id: 'aviation_english', label: 'Havacılık İngilizcesi' },
        { id: 'aviation_knowledge', label: 'Havacılık Bilgisi' },
        { id: 'communication', label: 'İletişim' },
      ],
    },
    {
      key: 'level',
      label: 'Seviye',
      type: 'select',
      required: true,
      defaultValue: 'B1',
      options: [
        { id: 'A1', label: 'A1' },
        { id: 'A2', label: 'A2' },
        { id: 'B1', label: 'B1' },
        { id: 'B2', label: 'B2' },
        { id: 'C1', label: 'C1' },
      ],
    },
    {
      key: 'category',
      label: 'Kategori',
      type: 'text',
      placeholder: 'vocabulary | grammar | reading | listening | phraseology | critical',
    },
    {
      key: 'format',
      label: 'Format',
      type: 'select',
      defaultValue: 'short',
      options: [
        { id: 'short', label: 'Kısa' },
        { id: 'passage', label: 'Pasaj' },
        { id: 'scenario', label: 'Senaryo' },
      ],
    },
    {
      key: 'question',
      label: 'Soru (EN)',
      type: 'textarea',
      required: true,
      rows: 3,
    },
    {
      key: 'question_tr',
      label: 'Soru (TR)',
      type: 'textarea',
      rows: 2,
    },
    {
      key: 'context',
      label: 'Bağlam (pasaj/dialog)',
      type: 'textarea',
      rows: 4,
    },
    {
      key: 'options',
      label: 'Şıklar (JSON)',
      type: 'json',
      required: true,
      rows: 6,
    },
    {
      key: 'correct_id',
      label: 'Doğru cevap (id)',
      type: 'text',
      required: true,
    },
    {
      key: 'weight',
      label: 'Ağırlık',
      type: 'number',
      defaultValue: 1,
    },
  ],
};

export const SCENARIO_SCHEMA: TableSchema = {
  table: 'scenarios',
  title: 'AI Senaryo',
  description: 'AI co-pilot ile yapılandırılmış konuşma senaryoları',
  slugFromKey: 'title',
  slugPrefix: 'scenario_{role}_{category}',
  revalidate: '/scenarios',
  fields: [
    {
      key: 'role',
      label: 'Rol',
      type: 'select',
      defaultValue: 'all',
      options: [
        { id: 'all', label: 'Tüm roller' },
        { id: 'pilot', label: 'Pilot' },
        { id: 'cabin', label: 'Kabin' },
        { id: 'technician', label: 'Teknisyen' },
        { id: 'ground', label: 'Yer hizmetleri' },
        { id: 'student', label: 'Öğrenci' },
      ],
    },
    {
      key: 'category',
      label: 'Kategori',
      type: 'select',
      defaultValue: 'atc',
      options: [
        { id: 'atc', label: 'ATC iletişim' },
        { id: 'cabin_emergency', label: 'Kabin acil durum' },
        { id: 'maintenance_call', label: 'Bakım çağrısı' },
        { id: 'gate_announcement', label: 'Kapı duyurusu' },
        { id: 'pre_flight', label: 'Uçuş öncesi' },
        { id: 'post_flight', label: 'Uçuş sonrası' },
        { id: 'irrops', label: 'IRROPS' },
        { id: 'medical', label: 'Tıbbi' },
        { id: 'security', label: 'Güvenlik' },
      ],
    },
    {
      key: 'title',
      label: 'Başlık (EN)',
      type: 'text',
      required: true,
    },
    {
      key: 'title_tr',
      label: 'Başlık (TR)',
      type: 'text',
    },
    {
      key: 'setup',
      label: 'Kurulum (EN)',
      type: 'textarea',
      rows: 3,
    },
    {
      key: 'setup_tr',
      label: 'Kurulum (TR)',
      type: 'textarea',
      rows: 3,
    },
    {
      key: 'initial_message',
      label: 'AI ilk mesaj',
      type: 'textarea',
      rows: 2,
      hint: 'Co-pilot AI senaryoyu bu cümleyle açar',
    },
    {
      key: 'goal',
      label: 'Hedef (EN)',
      type: 'textarea',
      rows: 2,
    },
    {
      key: 'goal_tr',
      label: 'Hedef (TR)',
      type: 'textarea',
      rows: 2,
    },
    {
      key: 'difficulty',
      label: 'Zorluk (1-5)',
      type: 'number',
      defaultValue: 3,
    },
    {
      key: 'estimated_minutes',
      label: 'Tahmini süre (dk)',
      type: 'number',
      defaultValue: 5,
    },
  ],
};

export const AIRLINE_SCHEMA: TableSchema = {
  table: 'airlines',
  title: 'Havayolu',
  description: 'Havayolu profili + role bazlı interview pipeline',
  slugFromKey: 'name',
  slugPrefix: 'airline',
  revalidate: '/airlines',
  fields: [
    {
      key: 'name',
      label: 'Ad',
      type: 'text',
      required: true,
    },
    {
      key: 'iata_code',
      label: 'IATA',
      type: 'text',
      placeholder: 'TK',
    },
    {
      key: 'country_emoji',
      label: 'Bayrak',
      type: 'text',
      placeholder: '🇹🇷',
    },
    {
      key: 'hub',
      label: 'Hub',
      type: 'text',
    },
    {
      key: 'region',
      label: 'Bölge',
      type: 'select',
      defaultValue: 'turkey',
      options: [
        { id: 'turkey', label: 'Türkiye' },
        { id: 'middle-east', label: 'Orta Doğu' },
        { id: 'europe-fsc', label: 'Avrupa FSC' },
        { id: 'europe-lcc', label: 'Avrupa LCC' },
        { id: 'asia', label: 'Asya' },
        { id: 'americas', label: 'Amerika' },
        { id: 'oceania', label: 'Okyanusya' },
        { id: 'africa', label: 'Afrika' },
      ],
    },
    {
      key: 'tier',
      label: 'Tier',
      type: 'select',
      defaultValue: 'flagship',
      options: [
        { id: 'flagship', label: 'Bayrak taşıyıcı' },
        { id: 'major', label: 'Büyük' },
        { id: 'lcc', label: 'LCC' },
        { id: 'regional', label: 'Bölgesel' },
        { id: 'cargo', label: 'Kargo' },
        { id: 'charter', label: 'Charter' },
      ],
    },
    {
      key: 'fleet_size',
      label: 'Filo sayısı',
      type: 'number',
    },
    {
      key: 'destinations',
      label: 'Destinasyon',
      type: 'number',
    },
    {
      key: 'prestige',
      label: 'Prestij (1-5)',
      type: 'number',
      defaultValue: 3,
    },
    {
      key: 'hiring_status',
      label: 'Alım durumu',
      type: 'select',
      defaultValue: 'open',
      options: [
        { id: 'open', label: 'Aktif alım' },
        { id: 'closed', label: 'Kapalı' },
        { id: 'open_day_only', label: 'Sadece Open Day' },
        { id: 'experienced_only', label: 'Sadece tecrübeli' },
      ],
    },
    {
      key: 'insider_tip_tr',
      label: 'İçeriden ipucu (TR)',
      type: 'textarea',
      rows: 3,
    },
    {
      key: 'pilot_interview',
      label: 'Pilot interview (JSON)',
      type: 'json',
      hint: '{ stages: [...], requiredLevel: "B2", englishWeight: 70 }',
      rows: 5,
    },
    {
      key: 'cabin_interview',
      label: 'Kabin interview (JSON)',
      type: 'json',
      rows: 5,
    },
  ],
};
