...
export default function ConversationScreen() {
  ...
  function evaluateTurn() {
    ...
    // FAZ 3 — must_include matching + store kaydı
    const dbTurn = (currentTurn as any) as { expected_response?: { must_include?: string[]; text_en?: string } };
    const mustInclude = dbTurn.expected_response?.must_include ?? [];
    const expectedText = dbTurn.expected_response?.text_en ?? currentTurn.expectedReadback ?? '';
    const lowerTranscript = transcript.toLowerCase();
    const hit: string[] = [];
    const miss: string[] = [];
    for (const kw of mustInclude) {
      if (lowerTranscript.includes(kw.toLowerCase())) hit.push(kw);
      else miss.push(kw);
    }
    useConversationPlayStore.getState().recordTurn({
      turnId: currentTurn.id,
      transcript,
      matchScore: result.score,
      mustIncludeHit: hit,
      mustIncludeMiss: miss,
      expectedResponse: expectedText,
    });
    ...
    track('conversation_turn_evaluated', {
      ...
      turn: currentTurn.id,
      score: result.score,
      must_include_hit: hit.length,
      must_include_miss: miss.length,
    });
  ...
  function finishScenario() {
    ...
    // FAZ 3 — result screen'e yönlendir
    useConversationPlayStore.getState().finish();
    setTimeout(() => {
      router.replace({
        pathname: '/conversation/[scenario]/result',
